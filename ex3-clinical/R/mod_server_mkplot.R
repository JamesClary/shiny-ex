...
mod_server_mkplot  <- function(id, simdat_raw){
  ...
  moduleServer(id, function(input, output, session){
    ...
    plot_outE <- eventReactive(input$btn, {
      ...
      if(input$nsim == 1){
        ...
        out_sum <- out_sum %>%
          reframe(plow = quantile(E, probs = probsc[1]),
                  pmid = quantile(E, probs = 0.5),
                  phi  = quantile(E, probs = probsc[2]),
                  .by = c(time)) |>
          mutate(Group = '')
    ...
    output$simPlotE <- renderPlot(plot_outE())

    plot_outDose <- eventReactive(input$btn, {
      ...
      if(input$nsim == 1){
        ...
        plot <- ggplot(out_sum %>% filter(Cc > 0) |> mutate(Group = ''))+
          geom_line(aes(time, DOSE, color = factor(Group), group = Group))+
          labs(x = 'Time Since First Dose (h)',
               y = 'Dose (mg)')+
          theme_bw()+
          theme(legend.position = 'none')
        ...
        out_sum <- out_sum %>%
          reframe(plow = quantile(DOSE, probs = probsc[1]),
                  pmid = quantile(DOSE, probs = 0.5),
                  phi  = quantile(DOSE, probs = probsc[2]),
                  .by = c(time)) |>
          mutate(Group = '')
    ...
    output$simPlotDose <- renderPlot(plot_outDose())
